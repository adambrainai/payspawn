...
contract PaySpawnBudgetPool {
    ...
    function addAgent(bytes32 credentialHash) external onlyOwner {
        registeredAgents[credentialHash] = true;
        emit AgentAdded(credentialHash);
    }

    function removeAgent(bytes32 credentialHash) external onlyOwner {
        registeredAgents[credentialHash] = false;
        emit AgentRemoved(credentialHash);
    }

    function addAgentsBatch(bytes32[] calldata credentialHashes) external onlyOwner {
